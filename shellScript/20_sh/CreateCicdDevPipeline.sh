...
CICD_TYPE=`echo $conf | jq -r '.Target[].CicdType'` && set | grep CICD_TYPE=
ACCOUNT_ID=`echo $conf | jq -r '.Target[].AccountId'` && set | grep ACCOUNT_ID=
ATF_BUCKET_NANE="gtc-cicddev-$ENVIRONMENT-$AREA-s3-cfn-cicd-artifactstore" && set | grep ATF_BUCKET_NANE=

bash Confirm.sh || exit 1

echo "Create InfraPipeline..."

aws cloudformation create-stack \
--stack-name gtc-system-deploy-cicd-$ENVIRONMENT-$AREA-cicdType-$CICD_TYPE \
--template-url https://s3.amazonaws.com/gtc-$ENVIRONMENT-$AREA-s3-cfn-cicd-infra/all-system-deploy-stack.yml \
--parameters \
...
if [ "$CICD_TYPE" = "dev" ]; then
  aws cloudformation create-stack \
  --stack-name gtc-system-develop-cicd-$ENVIRONMENT-$AREA-cicdType-$CICD_TYPE \
  --template-url https://s3.amazonaws.com/gtc-$ENVIRONMENT-$AREA-s3-cfn-cicd-infra/cicd-system-deploy-stack.yml \
  --parameters \
  ParameterKey=TemplateBucketName,ParameterValue=gtc-$ENVIRONMENT-$AREA-s3-cfn-cicd-infra \
  ParameterKey=AreaCode,ParameterValue=$AREA \
  ParameterKey=EnvCode,ParameterValue=$ENVIRONMENT \
  ParameterKey=CICDType,ParameterValue=$CICD_TYPE \
  ParameterKey=ArtifactStoreBucketName,ParameterValue=$ATF_BUCKET_NANE \
  ParameterKey=CodeBuildRole,ParameterValue=arn:aws:iam::$ACCOUNT_ID:role/codebuild-system-deploy-cicd \
  ParameterKey=CodePipelineRole,ParameterValue=arn:aws:iam::$ACCOUNT_ID:role/CodePipeline-system-deploy-cicd \
  ParameterKey=CloudWatchEventsRole,ParameterValue=arn:aws:iam::$ACCOUNT_ID:role/cwe-system-deploy-cicd \
  --capabilities CAPABILITY_NAMED_IAM \
